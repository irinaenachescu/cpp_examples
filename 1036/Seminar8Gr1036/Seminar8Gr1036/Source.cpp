...
public:
	...
	friend ostream& operator<<(ostream& os, Student student)
	{
		...
		os << "Numar de luni: " << student.numar_luni << endl;
...
Student* incarcaStudenti(char* nume_fisier, int& numar_studenti)
{
	Student* lista = NULL;
	ifstream fisier;
...
void genereazaRaport(char* nume_raport, Student* lista, int numar_studenti)
{
	ofstream fisier;
	fisier.open(nume_raport, ofstream::out);
	if (fisier.is_open())
	{
		for (int i = 0; i < numar_studenti; i++)
		{
			...
			{
				suma = suma + lista[i].buget[j];
			}
			fisier << "Buget: " << suma << endl;
		}
		fisier.close();
	}
	else
...
void main()
{
	...
	float buget[] = { 150, 250.6, 890 };
	Student s2("Popescu Ion", 24, 3, buget);
	cout << s2;

	Student* studenti = NULL;
	int numar_studenti=0;
	studenti = incarcaStudenti("Studenti.txt", numar_studenti);
	if (studenti != NULL)
	{
		for (int i = 0; i < numar_studenti; i++)
		{
	...
	genereazaRaport("Raport_studenti.txt", studenti, numar_studenti);
